...
function _hist_add!{T}(counts::Dict{T,Int}, labels::AbstractVector{T}, region::Range1{Int})
    for i in region
        lbl = labels[i]
...
function _hist_sub!{T}(counts::Dict{T,Int}, labels::AbstractVector{T}, region::Range1{Int})
    for i in region
        lbl = labels[i]
        counts[lbl] -= 1
    end
...
function _hist_shift!{T}(counts_from::Dict{T,Int}, counts_to::Dict{T,Int}, labels::AbstractVector{T}, region::Range1{Int})
    for i in region
        lbl = labels[i]
        counts_from[lbl] -= 1
...
_hist{T}(labels::AbstractVector{T}, region::Range1{Int} = 1:endof(labels)) =
...
nfoldCV_tree(labels::AbstractVector, features::AbstractMatrix, pruning_purity::Real, nfolds::Integer)                                           = _nfoldCV(:tree, labels, features, pruning_purity, nfolds)
nfoldCV_forest(labels::AbstractVector, features::AbstractMatrix, nsubfeatures::Integer, ntrees::Integer, nfolds::Integer, partialsampling=0.7)  = _nfoldCV(:forest, labels, features, nsubfeatures, ntrees, partialsampling, nfolds)
nfoldCV_stumps(labels::AbstractVector, features::AbstractMatrix, niterations::Integer, nfolds::Integer)                                         = _nfoldCV(:stumps, labels, features, niterations, nfolds)
...
function _nfoldCV{T<:Float64, U<:Real}(regressor::Symbol, labels::AbstractVector{T}, features::AbstractMatrix{U}, args...)
    nfolds = args[end]
    if nfolds < 2
        return nothing
    end
...
nfoldCV_tree{T<:Float64, U<:Real}(labels::AbstractVector{T}, features::AbstractMatrix{U}, nfolds::Integer, maxlabels::Integer=5)      = _nfoldCV(:tree, labels, features, maxlabels, nfolds)
nfoldCV_forest{T<:Float64, U<:Real}(labels::AbstractVector{T}, features::AbstractMatrix{U}, nsubfeatures::Integer, ntrees::Integer, nfolds::Integer, maxlabels::Integer=5, partialsampling=0.7)  = _nfoldCV(:forest, labels, features, nsubfeatures, ntrees, maxlabels, partialsampling, nfolds)
